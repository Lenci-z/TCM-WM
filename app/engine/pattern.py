# -*- coding: utf-8 -*-
"""
证型判定引擎（2.2）— 纯领域逻辑（P2-T2 解耦后不接 conn）
依据：设计方案 v0.2 第 2.1 节 轴二 + 2.5 辨证施养
逻辑：结构化四诊问卷勾选条目 → 与各证型 keywords 匹配计数
      最高分为主证，次高为兼证；医师可改判并确认（GUI 环节）。
输入：patterns（证型特征库列表，由 repo.get_patterns() 提供）+ selected_items
"""


def judge_pattern(patterns: list, selected_items: list):
    """证型判定。纯逻辑，无 conn（P2-T2）。
    参数：
      patterns: 证型特征库列表（由 repo.get_patterns() 获取，元素含 name/keywords）
    ...
    scores = {}
    for p in patterns:
        # 兼容键名：repo.get_patterns() 返回 pattern_name，旧结构用 name
        pname = p.get("name") or p.get("pattern_name")
        hits = [kw for kw in p.get("keywords", []) if kw in items]
        scores[pname] = len(hits)
...
if __name__ == "__main__":
    ...
    conn = get_conn()
    patterns = Repository(conn).get_patterns()
    cases = [
    ...
    for name, items in cases:
        main, sec, scores = judge_pattern(patterns, items)
